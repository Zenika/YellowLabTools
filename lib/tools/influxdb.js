...
async function write(results, options){
    ...
    writeApi.close()
        .then(() => {
            ...
            const jsonStream = new JsonStreamStringify(extractOffeders(results))
...
function extractOffeders(results){
    return results.map(data => {
        ...
        // making one object with all the offenders for one test
        return {
            url: data.params.url,
            name: data.params.options.name,
            offenders: Object.values(data.toolsResults)
                .map(toolResult => toolResult.offenders)
                .reduce((acc, offenders) => {
                    return {...acc, ...offenders}
                })
        }
    })
}
